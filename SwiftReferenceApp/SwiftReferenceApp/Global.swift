...
{ 
    ...
    { 
        do {
            try self.DOTDigraph.write(toFile: path, atomically: true, encoding: String.Encoding.utf8)
            return path
        } catch {
            return nil
        }
...
//    var array = [T]()
//    observable.subscribeOn  { array.append($0)  }
//    return array
//}  
...
extension Observable {
    ...
    func lastFor(dueTime: MainScheduler.TimeInterval) -> Observable<Element> {
        let delay = Observable<Int>.just(0)// .delaySubscription(dueTime, MainScheduler.sharedInstance)
