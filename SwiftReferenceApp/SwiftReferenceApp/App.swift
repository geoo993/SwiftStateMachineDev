...
//        userState = self._hsmTransitionState      >- map { (o, e, n, userState)      in return userState }
        ...
        appEvent    = self.transition.asObservable().map { t in return t.event }
        appState    = self.transition.asObservable().map { t in return t.newState }
        userState   = self.transition.asObservable().map { t in return t.userState }
